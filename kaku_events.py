...
def postUpdate(targetFile, action):
    ...
    if os.path.exists('%s.deleted' % targetFile):
        ...
        if action == 'deleted' and 'deleted' not in post:
...
def handlePost(eventAction, eventData):
    ...
    eventAction: created, updated, deleted or unchanged
    ...
    Post events generated by the gather daemon will have keys:
        gatheredPath, gatheredFilename
    """
    if eventAction == 'created':
        if 'gatheredPath' in eventData:
            postDir    = eventData['gatheredPath']
            targetFile = eventData['gatheredFile']
        else:
    ...
    elif eventAction in ('updated', 'deleted'):
        targetFile = eventData['gatheredFile']
        postUpdate(targetFile, eventAction)
    indexUpdate()
...
def handleEvent(eventKey):
    ...
    Valid Event Action are created, updated, deleted
