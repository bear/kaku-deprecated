...
def mention(sourceURL, targetURL, vouchDomain=None, vouchRequired=False):
    ...
    """
    current_app.log.info('discovering Webmention endpoint for %s' % sourceURL)

    mentions = ronkyuu.findMentions(sourceURL)
    result   = False
    vouched  = False
    current_app.log.info('mentions %s' % mentions)
    for href in mentions['refs']:
        if href != sourceURL and href == targetURL:
            current_app.log.info('post at %s was referenced by %s' % (targetURL, sourceURL))
            if vouchRequired:
    ...
    current_app.log.info('mention() returning %s' % result)
    ...
    if request.method == 'POST':
        ...
        if current_app.config.baseroute in target:
            ...
            if valid == requests.codes.ok:
                valid, vouched = mention(source, target, current_app.confg.site_config, vouch, current_app.config.vouch_required)
                if valid:
